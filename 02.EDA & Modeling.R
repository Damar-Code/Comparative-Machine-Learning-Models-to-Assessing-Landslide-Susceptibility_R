...
lmProfile <- rfe(x=trainData[,1:21], y=trainData$L.NL,
                 sizes = subsets,
                 rfeControl = ctrl)
...
trainData.VIF
lm.VIF <- lm(L.NL~Slope+Aspect+DtD+SPI+Elevation+
               TPI+RSP+CI+TWI+PlC+PrC,data=trainData.VIF)
lm.VIF
...
ggplot(data = df.VIF, 
  ...
  scale_fill_brewer(palette = "Set3") +
  geom_text(aes(label = round(VIF, 2)), vjust=1.6, color="black", size=4, fontface ="bold") + 
  theme_bw() + theme(legend.position = "none",
                     axis.text.x=element_text(colour="black", size = 15),
                     axis.text.y=element_text(colour="black", size = 15),
                     text = element_text(size=25))
...
write.csv(df.VIF, "E:/APRIL/Portofolio/Landslide Susceptibility Assesment using Machine Learning/table/multicolinearillity_removal.csv")
...
trainData_selected <- trainData %>%
  dplyr::select(c("L.NL", "Slope","Aspect","DtD","SPI","Elevation",
                    "TPI","RSP","CI","TWI","PlC","PrC"))
...
Selected.features <- c("Slope","Aspect","DtD","SPI","Elevation",
                       "TPI","RSP","CI","TWI","PlC","PrC")
...
rf_auc <- round(auc(ROC_test, rf.ROC),3)
rf_auc
# SVM
svm_roc <- roc(ROC_test, svm.ROC)
svm_auc <- round(auc(ROC_test, svm.ROC),3)
svm_auc
# XGBoost
XGb_roc <- roc(ROC_test, XGb.ROC)
XGb_auc <- round(auc(ROC_test, XGb.ROC),3)
XGb_auc
# Boosted Logistic Regression
Log_roc <- roc(ROC_test, Log.ROC)
Log_auc <- round(auc(ROC_test, Log.ROC),3)
Log_auc
...
### Let's see the Vriable Importance from XGBoost Model
varimp_XGBoost <- varImp(model_XGBoost)
plot(varimp_XGBoost, main="Variable Importance - XGBoost")
