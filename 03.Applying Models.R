...
Selected.features
list.Ras <- grep(list.files(pattern=".sdat$|.tif$|Slope-Percent|Aspect|DtD|DTM|SPI|Elevation|TPI|RSP|CI|TWI|PlC"), 
                 pattern='ORDER|Distance|Landform|PrC|Channel|Slope-Length|Slope_|.sgrd|.aux.xml|.mgrd|prj', invert=TRUE, value=TRUE)
...
df.bg <- as.data.frame(background_data)[1:10]
geometry <- as.data.frame(background_data)[11]
